...
- [Components](#components)
  ...
  - [Web application and UI](#web-application-and-ui)
    ...
    - [Party B (Fulfiller) view](party-b-placer-view)
  - [FHIR Server](#fhir-server)
  - [API management](#api-management)
  - [Policy engine](#policy-engine)
- [Example walk throughs](#example-walk-throughs)
  - [User onboarding](#user-onboarding)
  - [Creation of service request](#creation-of-a-service-request)
...
- List local patients
- List local service requests
- List tasks (local and external), possible filter by location, owner, status etc
- Search resources
- View of selected single (local and external) resources - as form & json
- Creation & update of local resources
- Creation of consents as defined by security profile
- Creation of task resource including assignment to partyB and linking to self owned service request
- Read external task and task change events
- Optional: Read questionaire resources, display as forms and write questionaire responses

#### Party B (Fulfiller) view

- List all local tasks assigned to partyB
- Read all external resources (i.e. service request) related to a task
- Create output resources such as appointments
- Read and update single task - status, outputs, owner etc..
- Optional: Create questionaires and read and display questionaire responses
...
- **Standard FHIR API (internal and external)**

This is the Standard FHIR API through which a user will manage his FHIR resources.

Note that this API may be exposed both to internal users as well as external users. This implies that it needs the capability to enforce security in different ways:

- Internal users: user bound access tokens.
- External users/clients: client/organiuation bound access tokens

Also URLs in response bodies to external clients may have to be rewritten to absolute URLs.

Arguably these APIs may be separated
...
This API acts as a proxy for the partner API when the user fetches resources from the externals party API as credentials are not available to the user agent (APP). The API identifies the target API and negotiates credentials based on the client credentials flow on injects the access token before forwarding. an URL replacement in the response payload may be needed before forwarding to the APP.
...
- Components based on available open-source frameworks:
  - FHIR store → HAPI
  - API gateway → Kong, APISIX, KrakenD?
